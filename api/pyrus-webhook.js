...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Достаём телефон, номер заявки
    const phone = String(fieldMap[FIELD_PHONE] || '').replace(/\D/g, '');
    const orderId = fieldMap[FIELD_ORDER_ID] || `TASK-${taskId}`;

    // 3. Ищем таблицу услуг (поле ID 9) и суммируем стоимость (колонка ID 13)
    let totalAmount = 0;
    const servicesTable = fields.find(f => f.id === 9);
    if (servicesTable && servicesTable.value) {
      console.log(`[TABLE] rows:`, JSON.stringify(servicesTable.value));
      // value — это массив строк, каждая строка — массив ячеек
      const rows = Array.isArray(servicesTable.value) ? servicesTable.value : [];
      rows.forEach((row, idx) => {
        if (Array.isArray(row)) {
          // ищем ячейку с id=13 (Стоимость)
          const costCell = row.find(c => c && c.id === 13);
